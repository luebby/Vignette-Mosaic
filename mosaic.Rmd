...
tally(F07_Aktien ~ G01_Geschlecht, format = "proportion",
      data = GKNW)
p_stipro <- prop(F07_Aktien ~ G01_Geschlecht, success = "richtig", data = GKNW)
p_stipro
diffp_stipro <-diffprop(F07_Aktien ~ G01_Geschlecht, success = "richtig", data = GKNW)
diffp_stipro
```
...
Bootvtlg <- do(1000) * diffprop(F07_Aktien ~ G01_Geschlecht, success = "richtig", 
                                           data = resample(GKNW))
gf_histogram( ~ diffprop, data = Bootvtlg, bins = 21, center = diffp_stipro) %>%
...
gf_histogram( ~ diffprop, data = Nullvtlg, bins = 21, center = 0) %>%
  ...
  gf_vline(xintercept = ~ diffp_stipro)

prop( ~ (abs(diffprop) >= abs(diffp_stipro)), data = Nullvtlg)
```
...
- Das $95\%$-Konfidenzintervall für $\pi_{\text{weiblich}}-\pi_{\text{maennlich}}$ überdeckt den Bereich $`r floor(confint(Bootvtlg)$lower*100)/100`$ bis $`r ceiling(confint(Bootvtlg)$upper*100)/100`$.
...
Haben diejenigen, die die (finanzwissenschaftliche) ökonomische Frage richtig beantwortet haben eine höhere Selbsteinschätzung der kognitiven Kompetenz bezüglich des Fachs Statistik?

## Grafisch

```{r, message=FALSE}
gf_boxplot(B_SATS_Kompetenz ~ F07_Aktien, data = GKNW) %>%
  gf_labs(title="SATS-36 und Öknomisches Wissensfrage",
          x="Öknomisches Wissensfrage", y="Kognitive Kompetenz") 

gf_violin(B_SATS_Kompetenz ~ F07_Aktien, data = GKNW) %>%
  gf_point(size = 3, stat = "summary") %>% 
  gf_labs(title="SATS-36 und Öknomisches Wissensfrage",
          x="Öknomisches Wissensfrage", y="Kognitive Kompetenz") 
```
...
```{r}
favstats(B_SATS_Kompetenz ~ F07_Aktien, data = GKNW)
mean_stipro <- mean(B_SATS_Kompetenz ~ F07_Aktien, data = GKNW)
mean_stipro
diffmean_stipro <-diffmean(B_SATS_Kompetenz ~ F07_Aktien, data = GKNW)
diffmean_stipro
```
...
```{r}
set.seed(1896)
Bootvtlg <- do(1000) * diffmean(B_SATS_Kompetenz ~ F07_Aktien, 
                                           data = resample(GKNW))
gf_histogram( ~ diffmean, data = Bootvtlg, bins = 21, center = diffmean_stipro) %>%
  gf_labs(title = "Bootstrap Verteilung Differenz Mittelwert Kompetenz") 
confint(Bootvtlg)
```

Permutationsverteilung unter $H_0: F_{\text{richtig}}=F_{\text{falsch}}$:

```{r}
set.seed(1896)
Nullvtlg <- do(1000) * diffmean(B_SATS_Kompetenz ~ shuffle(F07_Aktien), 
                                data = GKNW)
gf_histogram( ~ diffmean, data = Nullvtlg, bins = 21, center = 0) %>%
  gf_labs(title = "Permutationsverteilung Differenz Mittelwert Kompetenz") %>%
  gf_vline(xintercept = ~ diffmean_stipro)

prop( ~ (diffmean >= diffmean_stipro), data = Nullvtlg)
```

### Klassisch

```{r}
t.test(B_SATS_Kompetenz ~ F07_Aktien, alternative = "less",
       data = GKNW)
```

*Hinweis:* Anders als bei `diffmean()` bezieht sich hier das (einseitige) Konfidenzintervall auf $\mu_{\text{falsch}}-\mu_{\text{richtig}}$ und $H_0: \mu_{\text{falsch}} \geq \mu_{\text{richtig}} \Leftrightarrow \mu_{\text{falsch}} - \mu_{\text{richtig}} \geq 0$.

## Antwort


- In der Stichprobe liegt der Mittelwert der Selbsteinschätzung der kognitiven Kompetenz bei $\bar{x}_{\text{falsch}}=`r round(mean_stipro[1], 2)`$ und $\bar{x}_{\text{richtig}}=`r round(mean_stipro[2], 2)`$.
- Das $95\%$-Konfidenzintervall für $\mu_{\text{richtig}}-\mu_{\text{falsch}}$ überdeckt den Bereich $`r floor(confint(Bootvtlg)$lower*100)/100`$ bis $`r ceiling(confint(Bootvtlg)$upper*100)/100`$.
- Der p-Wert unter $H_0: \mu_{\text{richtig}}-\mu_{\text{falsch}} \leq 0 \Leftrightarrow \mu_{\text{richtig}} \leq \mu_{\text{falsch}}$ ist $`r prop( ~ (diffmean >= diffmean_stipro), data = Nullvtlg)`$.
...
Unterscheidet sich der Anteil der richtigen Antworten auf die ökonomische Frage zwischen den Altersklassen?

## Grafisch

```{r}
gf_bar( ~ F07_Aktien | ~  G02_Alter, data = GKNW)  %>%
  gf_labs(title="Öknomisches Wissensfrage",
  x="Antwort", y="Häufigkeit") 
```

```{r}
gf_bar( ~  G02_Alter, fill = ~ F07_Aktien, position = "fill",
        data = GKNW)  %>%
  gf_labs(title="Öknomisches Wissensfrage",
  x="Altersklasse", y="Anteil richtig/falsch") 
```

## Kennzahlen

```{r}
tally(F07_Aktien ~ G02_Alter, data = GKNW)
tally(F07_Aktien ~ G02_Alter, format = "proportion",
      data = GKNW)
p_stipro <- prop(F07_Aktien ~ G02_Alter, success = "richtig", data = GKNW)
p_stipro
```
...
```{r}
xchisq.test(F07_Aktien ~ G02_Alter, data = GKNW)
```


### Simulationsbasiert

Permutationsverteilung unter $H_0: F_{(18,23]}=F_{(23,28]}=F_{(28,33]}$:

```{r}
X.squared_stipro <- chisq.test(tally(F07_Aktien ~ G02_Alter, data = GKNW))$statistic
X.squared_stipro
set.seed(1896)
Nullvtlg <- do(1000) * chisq.test(tally(F07_Aktien ~ shuffle(G02_Alter), 
                                        data = GKNW))$statistic
gf_histogram( ~ X.squared, data = Nullvtlg, bins = 21, center = 0) %>%
  gf_labs(title = "Permutationsverteilung Chi-Quadrat") %>%
  gf_vline(xintercept = ~ X.squared_stipro)

prop( ~ (X.squared >= X.squared_stipro), data = Nullvtlg)
```
...
- In der Stichprobe liegt der Anteil richtiger Antworten bei $p_{(18,23]}=`r round(p_stipro[1], 2)`, p_{(23,28]}=`r round(p_stipro[2], 2)`$ und $p_{(23,33]}=`r round(p_stipro[3], 2)`$.
- Der p-Wert unter $H_0: \pi_{(18,23]}=\pi_{(23,28]}=\pi_{(28,33]}$ ist $`r prop( ~ (X.squared >= X.squared_stipro), data = Nullvtlg)`$.
...
Unterscheidet sich die Selbsteinschätzung der kognitiven Kompetenz bezüglich des Fachs Statistik zwischen den Altersklassen?

## Grafisch

```{r, message=FALSE}
gf_boxplot(B_SATS_Kompetenz ~ G02_Alter, data = GKNW) %>%
  gf_labs(title="SATS-36 und Alter",
          x="Alter", y="Kognitive Kompetenz") 

gf_violin(B_SATS_Kompetenz ~ G02_Alter, data = GKNW) %>%
  gf_point(size = 3, stat = "summary") %>% 
  gf_labs(title="SATS-36 und Alter",
          x="Alter", y="Kognitive Kompetenz") 
```
...
```{r}
favstats(B_SATS_Kompetenz ~ G02_Alter, data = GKNW)
mean_stipro <- mean(B_SATS_Kompetenz ~ G02_Alter, data = GKNW)
mean_stipro
```
...
```{r, message=FALSE}
erg_lm <- lm(B_SATS_Kompetenz ~ G02_Alter, data = GKNW)
library(car)
Anova(erg_lm)
```

*Hinweis*: die Funktion `Anova()` aus dem Paket `car` berechnet anders als die Funktion `aov()` einen marginalen (Typ-II) Test, `aov()` einen sequentiellen (Typ-I) Test. 
...
```{r}
F_stipro <- (do(1) * lm(B_SATS_Kompetenz ~ G02_Alter, data = GKNW))$F
F_stipro
set.seed(1896)
Nullvtlg <- do(1000) * lm(B_SATS_Kompetenz ~ shuffle(G02_Alter), data = GKNW)
gf_histogram( ~ F, data = Nullvtlg, bins = 21, center = 0) %>%
  gf_labs(title = "Permutationsverteilung F") %>%
  gf_vline(xintercept = ~ F_stipro)

prop( ~ (F >= F_stipro), data = Nullvtlg)
```
...
- In der Stichprobe liegen die Mittelwerte der Selbsteinschätzung der kognitiven Kompetenz bei $\bar{x}_{(18,23]}=`r round(mean_stipro[1], 2)`, \bar{x}_{(23,28]}=`r round(mean_stipro[2], 2)`$ und $\bar{x}_{(23,33]}=`r round(mean_stipro[3], 2)`$.
- Der p-Wert unter $H_0: \mu_{(18,23]}=\mu_{(23,28]}=\mu_{(28,33]}$ ist $`r prop( ~ (F >= F_stipro), data = Nullvtlg)`$.
...
Kann die erwartete Note durch die Selbsteinschätzung der kognitiven Kompetenz bezüglich des Fachs Statistik modelliert werden?
...
```{r}
gf_point(C4_Notenerwartung ~ B_SATS_Kompetenz, data = GKNW) %>%
  gf_lm() %>%
  gf_labs(x="Kognitive Kompetenz", y="Notenerwartung")
```

## Kennzahlen

```{r}
cor(C4_Notenerwartung ~ B_SATS_Kompetenz, data = GKNW)
```


## Modellierung + klassische Inferenz

```{r}
lm_stipro <- lm(C4_Notenerwartung ~ B_SATS_Kompetenz, data = GKNW)
summary(lm_stipro)
```
...
```{r}
beta_stipro <- coef(lm_stipro)[2]
set.seed(1896)
Bootvtlg <- do(1000) * lm(C4_Notenerwartung ~ B_SATS_Kompetenz, 
                                           data = resample(GKNW))
gf_histogram( ~ B_SATS_Kompetenz, data = Bootvtlg, bins = 21, center = beta_stipro) %>%
  gf_labs(title = "Bootstrap Verteilung Steigung Kognitive Kompetenz") 
confint(Bootvtlg)
```

```{r, include=FALSE}
confint(Bootvtlg$B_SATS_Kompetenz)[1]
```
...
```{r}
set.seed(1896)
Nullvtlg <- do(1000) * lm(C4_Notenerwartung ~ shuffle(B_SATS_Kompetenz), 
                                data = GKNW)
gf_histogram( ~ B_SATS_Kompetenz, data = Nullvtlg, bins = 21, center = 0) %>%
  gf_labs(title = "Permutationsverteilung Steigung Kognitive Kompetenz") %>%
  gf_vline(xintercept = ~ beta_stipro)

prop( ~ (abs(B_SATS_Kompetenz) >= abs(beta_stipro)), data = Nullvtlg)
```

## Antwort

-  Die geschätzte Modellgleichung lautet:

$$\widehat{\text{Notenerwartung}}=`r round(coef(lm_stipro)[1],2)`  `r round(coef(lm_stipro)[2],2)` \cdot \text{Eingeschätzte Kompetenz}$$

-  Modellgüte $R^2=`r round(rsquared(lm_stipro),2)`$.

-  Das $95\%$-Konfidenzintervall für $\beta_{\text{Eingeschätzte Kompetenz}}$ überdeckt den Bereich $`r floor(confint(Bootvtlg$B_SATS_Kompetenz)[1]*100)/100`$ bis $`r ceiling(confint(Bootvtlg$B_SATS_Kompetenz)[2]*100)/100`$.

- Der p-Wert unter $H_0: \beta_{\text{Eingeschätzte Kompetenz}}=0$ ist $\leq `r tally(  ~ (abs(B_SATS_Kompetenz) >= abs(beta_stipro)), data = Nullvtlg)[1]+1`/`r nrow(Nullvtlg)`$.

*Hinweis*: Im Regressionsmodell können Kovariablen etc. berücksichtigt werden und viele Inferenzmethoden lassen sich integrieren: [https://lindeloev.github.io/tests-as-linear/](https://lindeloev.github.io/tests-as-linear/)
...
- `mosaic`: [https://projectmosaic.github.io/mosaic/](https://projectmosaic.github.io/mosaic/)
- `ggformula`: [https://projectmosaic.github.io/ggformula/](https://projectmosaic.github.io/ggformula/)
